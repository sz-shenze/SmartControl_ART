...
bool TouchGFXGeneratedHAL::blockCopy(void* RESTRICT dest, const void* RESTRICT src, uint32_t numBytes)
{
    ...
    if (dataOffset >= 0x9A000000 && dataOffset < 0xA0000000)
    {
        ...
        if(rt_device_find("sd0") != RT_NULL)
        {
            ...
            {
                fd = open("/sdcard/Resource/image/images.bin", O_RDONLY, 0);
                if (fd < 0)
                {
                    rt_kprintf("open file for read failed\n");
                    rt_memset((uint8_t *)dest, 0x00, numBytes);
                    return true;
                }
...
//    uint32_t dataOffset = (uint32_t)src;
//    if (dataOffset >= 0x9A000000 && dataOffset < 0x9B400000)
//    {
//            int fd;
//            struct statfs buffer;
//            if ((dfs_statfs("/sdcard",&buffer) == RT_EOK))
//            {
//                fd = open("/sdcard/Resource/image/images.bin", O_RDONLY, 0);
//                if (fd < 0)
//                {
